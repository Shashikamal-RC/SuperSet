        ...
        if st.button("✅ Confirm & Proceed", key="confirm_proceed"):
            ...
            with st.spinner("Posting Job at SuperSet..."):
                automator = SupersetAutomator(
                    ...
                    password="@Mesa2025",                    headless=os.getenv("HEADLESS", "False").lower() in ("true", "1", "t"),
                    browser=os.getenv("BROWSER", "chrome").lower(),
                )
                job_posting_success = automator.run(job_data_obj)
                
                if job_posting_success:
                    st.success("📡 Job post successful!")
                    
                    # Display application deadline if available
                    if hasattr(job_data_obj, 'application_deadline') and job_data_obj.application_deadline:
                        st.info(f"📅 **Application Deadline:** {job_data_obj.application_deadline}")
                        
                        # Store deadline in session state for persistence
                        st.session_state.application_deadline = job_data_obj.application_deadline
                else:
            ...
            if job_posting_success:
                ...
                with st.spinner("Logging to Google Sheet..."):
                    ...
                    logging_success = log_job_data(job_data_obj)
                    if logging_success:
            ...
            if logging_success:
                # Option to send Slack notification
                with st.spinner("Sending Slack notification..."):                
                    ...
                    # Add application deadline to the notification if available
                    if hasattr(job_data_obj, 'application_deadline') and job_data_obj.application_deadline:
                        message += f"\nApplication Deadline: {job_data_obj.application_deadline}"
